...
int cmd_rmdir(cmd_tast *t, MYSQL *p_mysql)
{
    ...
    int size = 0;
    // 调用函数分割字符串
    char **result = splitStringBySpace(t->content, &size);
    ...
    if (no_match_found_count != 0)
    {
        ...
        strcat(t->content, "failed: There is no file or directory for that.\n");
    }
    // printf("%s\n", t->content);
    if (non_empty_count != 0)
    {
        ...
        strcat(t->content, "failed: Nonempty directory.");
    }
    // printf("%s\n", t->content);
    // 释放动态分配的内存
    for (int i = 0; i < size; i++)
